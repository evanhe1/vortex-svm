...
Emulator::ipdom_entry_t::ipdom_entry_t(const ThreadMask &tmask, Word PC)
    : tmask(tmask), PC(PC), fallthrough(false)
{
}
...
Emulator::warp_t::warp_t(const Arch &arch)
    : ireg_file(arch.num_threads(), std::vector<Word>(arch.num_regs())), freg_file(arch.num_threads(), std::vector<uint64_t>(arch.num_regs()))
{
}
...
Emulator::Emulator(const Arch &arch, const DCRS &dcrs, Core *core)
    : arch_(arch), dcrs_(dcrs), core_(core), warps_(arch.num_warps(), arch), barriers_(arch.num_barriers(), 0)
{
  this->clear();
}
...
void Emulator::clear()
{
  // TODO: gpu execution starts here, read DCR for SATP here
  // write to CSR here
...
#define CSR_READ_64(addr, value)                       \
  ...
  case (addr + (VX_CSR_MPM_BASE_H - VX_CSR_MPM_BASE)): \
...
Word Emulator::get_csr(uint32_t addr, uint32_t tid, uint32_t wid)
{
  ...
  case VX_CSR_THREAD_ID:
    return tid;
  case VX_CSR_WARP_ID:
    return wid;
  case VX_CSR_CORE_ID:
    return core_->id();
  case VX_CSR_ACTIVE_THREADS:
    return warps_.at(wid).tmask.to_ulong();
  case VX_CSR_ACTIVE_WARPS:
    return active_warps_.to_ulong();
  case VX_CSR_NUM_THREADS:
    return arch_.num_threads();
  case VX_CSR_NUM_WARPS:
    return arch_.num_warps();
  case VX_CSR_NUM_CORES:
    return uint32_t(arch_.num_cores()) * arch_.num_clusters();
  case VX_CSR_NUM_BARRIERS:
    return arch_.num_barriers();
  case VX_CSR_MSCRATCH:
    return csr_mscratch_;
    CSR_READ_64(VX_CSR_MCYCLE, core_perf.cycles);
    CSR_READ_64(VX_CSR_MINSTRET, core_perf.instrs);
  default:
    if ((addr >= VX_CSR_MPM_BASE && addr < (VX_CSR_MPM_BASE + 32)) || (addr >= VX_CSR_MPM_BASE_H && addr < (VX_CSR_MPM_BASE_H + 32)))
    {
